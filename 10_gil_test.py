...
def threaded_count_primes(prime_number: int, num_threads: int) -> int:
    ...
    step = prime_number // num_threads
    for i in range(num_threads):
        start = i * step
        end = (i + 1) * step if i != num_threads - 1 else prime_number
...
def multiprocess_count_primes(prime_number: int, num_processes: int) -> int:
    with multiprocessing.Pool(processes=num_processes) as pool:
        step = prime_number // num_processes
        tasks = [
            (i * step, (i + 1) * step if i != num_processes - 1 else prime_number)
...
def main() -> None:
    '''
    with gil enabled
        Single-threaded: 78498 primes in 3.33 seconds
        Threaded: 78498 primes in 3.90 seconds
        Multiprocessed: 78498 primes in 1.61 seconds
    '''
    ...
    # SYNCHRONOUS Single-threaded part
